...
class HGTLayer(nn.Module):
    ...
    def forward(self, G, h):
        with G.local_scope():
            ...
            canons = G.canonical_etypes
            for srctype, etype, dsttype in G.canonical_etypes:
                sub_graph = G[srctype, etype, dsttype]

                nodes_for_test = sub_graph.nodes
                ...
                target_index_interested = 148
                target_indices = edges[1] == target_index_interested

                sums = torch.sum(attn_score[target_indices], dim=0)

                sub_graph.edata["t"] = attn_score.unsqueeze(-1)

            G.multi_update_all(
                {
                    etype: (fn.u_mul_e("v", "t", "m"), fn.sum("m", "t"))
